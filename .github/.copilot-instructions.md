...
│   ├── challenges/      # Défis (V2)
│   └── settings/        # Paramètres (avec sélection de thème)
├── context/
│   └── ThemeContext.tsx # Contexte de thème (sans rechargement)
├── api/                 # API Routes
│   └── auth/
├── lib/                 # Utilitaires
│   ├── prisma.ts       # Client Prisma singleton
│   ├── actions.ts      # Server actions
│   ├── themes.ts       # Configuration des 5 thèmes
│   └── data.ts         # Fonctions de récupération
└── ui/                 # Composants réutilisables ⭐ RÉORGANISÉ
    ├── common/          # Composants réutilisables (button, search, etc.)
    ├── forms/           # Formulaires (login, signup)
    ├── modals/          # Modales (level-up, rewards)
    ├── animations/      # Composants d'animation + CSS
    ├── icons/           # Logos et icônes
    ├── layouts/         # (Réservé pour futurs layouts)
    ├── activities/      # Composants spécifiques
    ├── badges/          # Badges
    ├── calendar/        # Calendrier
    ├── challenges/      # Défis
    ├── charts/          # Graphiques
    ...
    ├── gallery/         # Galerie
    ├── settings/        # Paramètres
    ├── global.css       # Styles globaux + thèmes
...
## 🎨 Système de thèmes (✅ CORRIGÉ - Pure Tailwind)

### Thèmes disponibles
L'application dispose de **5 thèmes** qui s'appliquent **globalement** et **instantanément** :

1. **Light** (Rose) - Niveau 1 requis - Thème par défaut
...
### Configuration technique (Approche Pure Tailwind)

- **global.css** (`app/ui/global.css`) :
  - CSS minimal et pur (pas de `!important` partout)
  - Classes de thème simples (`.theme-dark`, `.theme-sunset`, etc.)
  - Gradients de fond pour chaque thème
  - Transitions fluides natives CSS
  - **Total : ~80 lignes** contre 620 lignes avant ! ✨
...
- **Layout** (`app/layout.tsx`) :
  - Récupération du thème utilisateur depuis la DB
  - Application initiale des classes de thème
  - ThemeProvider enveloppe l'application

### Utilisation dans les composants

**Approche Tailwind recommandée :** Utiliser les classes Tailwind standard et laisser le thème s'appliquer automatiquement via le `color-scheme` CSS et les classes de contexte.

```tsx
// ✅ BON - Classes Tailwind normales
...
// ✅ BON - Utiliser les variantes dark: si besoin
<div className="bg-white dark:bg-slate-800">
  <p className="text-gray-900 dark:text-slate-100">Texte</p>
</div>

// ❌ ÉVITER - Pas besoin de conditions complexes
{theme === 'dark' ? 'bg-slate-900' : 'bg-white'}
```

### Styling avec thèmes

- Les thèmes appliquent automatiquement les fonds et `color-scheme`
- Utiliser Tailwind normalement, le thème gère le contexte
- Les classes conditionnelles `dark:` fonctionnent automatiquement
- Préférer les classes sémantiques (`bg-white`, `text-gray-900`)
- Les gradients sont définis par thème dans `global.css`